...
int main(int argc, char** argv)
{
  ...
  float32 single_step_time = 0.0;
  float32 collision_step_time = 0.0;
  float32 velocity_update_step_time = 0.0;
  float32 position_update_step_time = 0.0;
  float32 loop_time;
  timeval t1, t2;
  
  gettimeofday(&t1,NULL); //! Timer starts here
  ...
  gettimeofday(&t2,NULL); //! Timer ends here
  
  loop_time = (t2.tv_sec - t1.tv_sec) * 1000.0;     
  loop_time += (t2.tv_usec - t1.tv_usec) / 1000.0;
  std::cout << "Number of Iterations: " << loops << "\n";
  std::cout << "Average time per step is " <<  single_step_time/loops <<  " ms\n";
  std::cout << "Average time for collisions is " << collision_step_time/loops << " ms\n";
  std::cout << "Average time for velocity updates is " << velocity_update_step_time/loops << " ms\n";
  std::cout << "Average time for position updates is " << position_update_step_time/loops << " ms\n";
  std::cout << "Total loop time is " << loop_time << " ms\n";
 */ 
  return 0;
}
