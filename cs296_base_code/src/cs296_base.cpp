...
base_sim_t::base_sim_t()
{
  ...
  /// Gear Index : Tells which gear you are on currently
...
/*
void base_sim_t::myfunction(){
  std::cout << "here\n";
  b2Vec2 vel = sbody2->GetLinearVelocity();
  if (body->GetAngularVelocity() > -5.0f){
      body->SetAngularVelocity(body->GetAngularVelocity() - 0.2f);
    }

  sbody1->SetLinearVelocity(b2Vec2(1.0f,0.0f));
  sbody2->SetLinearVelocity(b2Vec2(vel.x + 1.0f, vel.y));
}

void base_sim_t::incrementGear(){
  {
      if (gearIndex < 4){
        b2CircleShape shape;
        gearIndex = 4.0f - (gearIndex - 1) * 0.5f;
...
void base_sim_t::decrementGear(){
  {
      if (gearIndex >= 1){
        m_world->DestroyBody(gear);
      }
  }
}
*/